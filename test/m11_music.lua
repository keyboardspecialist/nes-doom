-- M11: compiled D_E1M1 drives all five tonal voices plus noise-only drums.
local frames = 0
local MT = nil
local writes = {p0 = 0, p1 = 0, tri = 0, m0 = 0, m1 = 0, noise = 0}
local periods = {p0 = {}, p1 = {}, tri = {}, m0 = {}, m1 = {}}
local musicBanks, pistolSounds = 0, 0
local mmc5Enabled, dmcEnabled = false, false
local voicesChecked = false
local noiseLengthHalted = false
local periodicNoise = false
...
pcall(function()
  ...
  emu.addMemoryCallback(function(_addr, value)
    if value >= 0x10 then dmcEnabled = true end
  ...
  emu.addMemoryCallback(function(_addr, value)
    if value >= 0x80 then periodicNoise = true end
  end, emu.callbackType.write, 0x400E)
end)

emu.addEventCallback(function()
  emu.setInput({start = frames < 10, a = frames >= 650 and frames < 653,
                right = frames >= 300 and frames < 360}, 0)
...
emu.addEventCallback(function()
  ...
  if frames == 800 then
    ...
    if dmcEnabled then return fail("DMC channel was enabled") end
    if noiseLengthHalted then return fail("music noise halted its length counter") end
    if periodicNoise then return fail("music used pitched periodic noise") end
  ...
  elseif frames == 5900 then
    ...
    print(string.format(
      "M11 PASS (loop=1 banks=%d tonal=%d/%d/%d/%d/%d noise=%d)",
      musicBanks, writes.p0, writes.p1, writes.tri,
